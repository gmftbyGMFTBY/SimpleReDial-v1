...
class BERTDualRandomEncoder(nn.Module):

    def __init__(self, **args):
        super(BERTDualRandomEncoder, self).__init__()
    ...
    def forward(self, batch):
        ...
        rid = batch['rids']
        scid = batch['sids']
        scid_mask = batch['sids_mask']
        cid_mask = batch['ids_mask']
        rid_mask = batch['rids_mask']

        cid_rep, rid_rep = self._encode(cid, rid, cid_mask, rid_mask)
        scid_rep = self.ctx_encoder(scid, scid_mask)
        # gather all the embeddings in other processes
        # cid_rep, rid_rep = distributed_collect(cid_rep, rid_rep)

        cid_rep = torch.cat([cid_rep, scid_rep], dim=0)
