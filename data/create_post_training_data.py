...
class CreateBertPretrainingData(object):
    ...
    def create_training_instances(self, input_file, max_seq_length,
        ...
        self.feature_keys = ["input_ids", "attention_mask", "token_type_ids", "masked_lm_positions", "masked_lm_ids", "next_sentence_labels"]
    ...
    def build_h5_data(self, hf, d_idx=0):
        ...
        if d_idx == 0:
            for feat_key in self.feature_keys:
                ...
                h5_key_dict[feat_key] = hf.create_dataset(
                    feat_key, tuple(key_size), dtype='i8', chunks=True,
    ...
    def create_instances_from_document(self, all_documents, document_index, max_seq_length, short_seq_prob, masked_lm_prob, max_predictions_per_seq, vocab_words, rng):
    ...
    def create_masked_lm_predictions(self, tokens, masked_lm_prob, max_predictions_per_seq, vocab_words, rng):
        ...
        for (i, token) in enumerate(tokens):
            ...
            if (self.args.do_whole_word_mask and len(cand_indexes) >= 1 and token.startswith("##")):
...
if __name__ == "__main__":
    ...
    create_data.create_training_instances(
        ...
        args.max_predictions_per_seq, rng, args.special_tok
    )
